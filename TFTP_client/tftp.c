...
void send_file(int sockfd, struct sockaddr_in client_addr,socklen_t client_len, char *filename)
{
    ...
    while (1)
    {
        ...
        /* Read data based on transfer mode */
        if (transfer_mode == MODE_OCTET)
        {
            /* OCTET mode sends 1 byte at a time */
            bytes = read(fd, pkt.body.data_packet.data, 1);
        }
        else
        {
            /* DEFAULT and NETASCII send 512 bytes */
            bytes = read(fd, pkt.body.data_packet.data, 512);
        }

        pkt.body.data_packet.data_size = bytes;

        /* NETASCII conversion while sending data */
        if(transfer_mode == MODE_NETASCII && bytes > 0)
        {
            if(pkt.body.data_packet.data[0] == '\n')
            {
                pkt.body.data_packet.data[0] = '\r';
            }
        }
